...
mumps_Nmif <-        c(10, 200,400)
mumps_Nreps_eval <-  c(2,  10,  20)
...
stew(file=sprintf("global0508.rda",run_level),{
  global.result<- foreach(i=1:mumps_Nreps_global[run_level], .combine=rbind,
                          ...
                          .inorder=FALSE, .options.multicore=mcopts) %dopar%  {
                            ...
                            mif2(origin.model, start=start.parameter, Np=mumps_Np[run_level],
                                 Nmif=mumps_Nmif[run_level],
                                 cooling.type="geometric",cooling.fraction.50=0.1, transform=TRUE,
                                 rw.sd=rw.sd( 
                                   R0=rw.sd_rp,mu=rw.sd_rp,sigma=rw.sd_rp,gamma=rw.sd_rp,
                                   alpha=rw.sd_rp,iota=rw.sd_rp, rho=rw.sd_rp,
                                   sigmaSE=rw.sd_rp, psi=rw.sd_rp, pampm10=rw.sd_rp,
                                   pampm25=rw.sd_rp,pamso2=rw.sd_rp,pamno2=rw.sd_rp,
                                   pamco=rw.sd_rp,pamo3=rw.sd_rp,
                                   I_0=ivp(rw.sd_ivp),
                                   S_0=ivp(rw.sd_ivp),
                                   E_0=ivp(rw.sd_ivp),
                                   R_0=ivp(rw.sd_ivp)
                                 )) -> mf   ##  %>%  mif2() 
                            pf <- replicate(mumps_Nreps_eval[run_level], pfilter(mf, Np = mumps_Np[[run_level]]))
                            ll <- sapply(pf,logLik)
