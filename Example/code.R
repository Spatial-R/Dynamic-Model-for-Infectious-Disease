...
rproc <- Csnippet("
                  ...
                  double   birthnum    = rpois(0.32*birthrate*dt);		     // births are Poisson
                  
                //   double   vaccinum    = rpois(vacci*dt);      // vaccination are Poisson
                  
                  double   vaccinum    = birthnum *2 ;  // 80% vaccine, 85% effeief

                  rate[0] = exp(pm25 * pam2pm25)*lambda * dt;  // stochastic force of infection with the effects on susceptible
...
Csnippet("
         ...
         double   birthnum    = rpois(0.32*birthrate);		     // births are Poisson
         double   vaccinum    = birthnum *2;  // 80% vaccine, 85% effeief

         rate[0] = exp(pm25 * pam2pm25)*lambda  ;  // stochastic force of infection
...
#### 死亡率5.2，出生率10.6
c(R0=5,gamma=7/20,sigma=7/15,mu=0.20e-03,sigmaSE=0.512,
  rho=0.85, alpha= 0.976, iota=2.9, psi=0.116,effcontact=0.05,
  logbeta1=0.67,
  pam1pm25=0.01,#pam1temp=0.002, #pam1pm25=0.002
  pam2pm25=0.01,#pam2temp=0.002, #pam2pm25=0.002,
#  pam3pm25=0.01,#pam3temp=0.002, #pam3pm25=0.002,
...
varicella.hz %>%
  pomp(t0=varicella.hz$week[1],
       ...
       paramnames=c("logbeta1","mu","sigma","gamma","alpha","iota","psi","effcontact",
                    "rho","sigmaSE","pam1pm25",#"pam1temp",  ###,"pam1pm25",
                    "pam2pm25",#"pam2temp",                  ### "pam2pm25",
                   # "pam3pm25",#"pam3temp",                  ### "pam3pm25",
                    "S_0","E_0","I_0","R_0")
  ) -> origin.model
...
stew(file=sprintf("globalpm250425.rda",run_level),{
  global.result<- foreach(i=1:500, .combine=rbind,
                          ...
                          .inorder=FALSE, .options.multicore=mcopts) %dopar%  {
                            ...
                            mif2(origin.model, start=start.parameter, Np=5000,
                                 Nmif=100,
                                 cooling.type="geometric",cooling.fraction.50=0.1, transform=TRUE,
                                 rw.sd=rw.sd( 
                                   logbeta1=0.02,mu=0.02,sigma=0.02,gamma=0.02,
                                   alpha=0.02,iota=0.02, rho=0.02,
                                   sigmaSE=0.02, psi=0.02, pam1pm25=0.02,
                                   pam2pm25=0.02,effcontact=0.02,
                                  # pam1temp=0.02,
                                 #  pam2temp=0.02,
                                 #  pam2temp=0.02,
                                 # pam3pm25=0.02, 
...
write.csv(global.result,file="global.resultpm250425.csv",row.names=F)
