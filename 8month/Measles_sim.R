...
t                             =       0
R_0                           =       4.2
time.step                     =       3       # number of days to step forward
...
initial.prop.susceptible      =       intial[,3]*3    ##### the percentage of susceptible
...
for(j in 1:num.steps){
  ...
  beta_1                        =       1.8
  N                            <-      sum(demographic.ages[22:98,2])
  new.infected                =       0
  beta.2                        =       beta_0 * ( 1 + beta_1 * cos( 2 * pi * (j*time.step)) )
  ...
  for ( i in 1:length(demographic.ages[,1]) ){
    ...
    foi.age                 =      force.of.infection.by.age(age,mixing.matrix,disease.state,beta.2,gamma,time.step,inf.comp,num.comps)
  ...
  t                        =       t   +    time.step
  demographic.ages$Number<-apply(data.frame(matrix(disease.state,ncol=6,byrow = T)),1,sum)  ###reflesh the data
